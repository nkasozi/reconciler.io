...
describe('inferDataType', () => {
	test('recognizes numbers with commas and parentheses as numbers', () => {
		expect(inferDataType('1,000')).toBe('number');
		expect(inferDataType('1,000.08')).toBe('number');
		// Parentheses are often used for negatives in spreadsheets
		expect(inferDataType('(1,000)')).toBe('number');
		// Leading/trailing whitespace
		expect(inferDataType('  2,345  ')).toBe('number');
	});

	test('falls back to string for non-numeric values', () => {
		expect(inferDataType('abc')).toBe('string');
